...
export function parseKeyPacket(buf) {
    if (!Buffer.isBuffer(buf) || buf.length < KEY_HEADER_BYTES)
        return null;
    if (buf.readUInt8(0) !== MsgType.Key)
        return null;
    if (buf.readUInt8(1) !== PROTOCOL_VERSION)
        return null;
    const action = buf.readUInt8(2);
    const keyLen = buf.readUInt16LE(3);
    if (KEY_HEADER_BYTES + keyLen > buf.length)
        return null;
